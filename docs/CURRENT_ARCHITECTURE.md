...
| api image mein Go/node hi nahi tha | `backend-go/Dockerfile` | Runtime image alpine + git + rsync thi. `go build` "executable not found" deta tha, jise code galat hone se alag nahi kiya jaa raha tha. Nateeja: `taskComplete` **kabhi** true nahi ho sakta tha (har task poori 5 iterations jalata tha), aur `publishCodeArtifacts` build-fail pe return karta tha toh **ek bhi code artifact blackboard pe nahi pahunchta tha**. |
| `go build` React expert pe bhi chalta tha | naya `internal/workflow/verify.go` | Frontend expert ko Go ka build error fix karne bola jaata tha. Ab verification **workspace ke manifest se** decide hoti hai (`go.mod` → Go, `package.json` → node), expert ke label se nahi. |
| "Build Errors:" khaali body ke saath | `observeWorkspace` | exec ka not-found error koi output nahi deta, toh model ko heading milti thi aur neeche kuch nahi. Ab missing tool "could not be verified — NOT a problem in your code" bolta hai. |
| Model apni hi files nahi dekh paata tha | `observeWorkspace` | Sirf `git status --short` bhejta tha, aur `auto_commits` ke kaaran wo hamesha khaali hota tha. Ab poori file list jaati hai. |
| Wahi RAG query 5 baar | `runAiderLoop` | `loadExpertTraining` har iteration pe chalti thi, jabki task description badalti nahi — 5 identical embed + pgvector calls per expert. Ab ek baar. |
